...
【出力JSONフォーマット】
{
  ...
  "narrative": {
    "story": {
      "aim": "<今日の狙い、60字以内。カープがこの試合で何をしようとしていたか。先発起用・打線構成・戦術の意図>",
      "pivot": "<試合を動かした分岐、80字以内。試合の意味や主導権が変わった具体的な場面（得点/失点/継投/采配）>",
      "answer": "<得られた答え、80字以内。狙いが成立したか。機能した選手・戦術を事実ベースで>",
      "issue": "<残った課題、80字以内。この試合で解決しなかった問題。次に持ち越すもの>"
    },
...
- aim（狙い）: 首脳陣の意図に触れるが**断定しない**。「〜を狙ったと見られる」「〜する意図があった可能性」のように可能性として書く
- pivot（分岐）: スコアが動いた場面だけでなく「試合の意味・主導権が変わった場面」を優先。事実ベースで
- answer（答え）: 狙いが成立したかを事実で。機能した選手名・戦術を一球速報/moments から具体的に
- issue（課題）: team_analysis.issues と整合させ、次に持ち越す問題を1〜2点に絞る
- next_question: issue から自然につながる「次戦で確かめたい問い」。断定を避け問いの形で
- 全体を通して、事実（何が起きたか）と解釈（何を意味するか）を混同しない。首脳陣の意図は複数の可能性がありうる前提で書く
`;
}

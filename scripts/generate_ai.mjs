...
function buildPrompt(facts) {
  ...
  const factsBlob = JSON.stringify({
    ...
    recentLineups: f?.recentLineups || null,  // 直近3試合の打順（打順変化の意図を書くため）
    carpBatting: f?.carpLineup,  // 本塁打数など数値の厳守用（lineup と同じだが明示）
    carpBatterHands: f?.carpBatterHands || null,  // 打者の左右（例:「右投左打」）
    keyMatchups: f?.keyMatchups || null,  // 相手先発との相性（今季対戦成績）
  }, null, 2);
...
【出力JSONフォーマット】
{
  ...
  "narrative": {
    "story": {
      "aim": "<今日の狙い、100字以内。カープがこの試合で何をしようとしていたか。打順の組み方（誰を何番に置いたか・その意図）、先発起用、相手先発の特徴に対する打線の当て方まで具体的に。断定せず『〜狙いと見られる』の形で>",
      "pivot": "<試合を動かした分岐、120字以内。試合の主導権が変わった場面を、采配のプロセスとともに。途中起用（代打・代走・守備固め）、継投のタイミング、相手打順との噛み合わせがどう影響したかを具体的に>",
      "answer": "<得られた答え、100字以内。狙いが成立したか。機能した／しなかった打順・起用・継投を、実際の打席結果を根拠に。誰のどの場面が効いた（効かなかった）かまで>",
      "issue": "<残った課題、100字以内。打順・起用・継投・攻め方の面で解決しなかった問題。次に持ち越すもの>"
    },
    "next_question": "<次の試合への問い、50字以内。この試合の課題を踏まえ、次戦で確かめたくなる問い。「〜か」で終える>"
  }
}
...
- ★数値の厳守★: 各選手の本塁打数・安打数・打点・打数は、入力の carp_batting の数値を**そのまま**使う。勝手に増減させない。特に「◯ラン（=打点）」と「◯本塁打（=本数）」を混同しない（例: 8回に2ランを打った＝本塁打1本・打点2。これを「2本塁打」と書くのは誤り）。ある選手の本塁打数は carp_batting の hr の値と厳密に一致させること
...
【narrative（今日のストーリー）のルール】
- story は「試合の要約」ではなく、狙い→分岐→答え→課題を**因果でつなぐ**こと。単なる出来事の羅列にしない
- ★結果論を避ける★: 「打てなかった」「勝った/負けた」だけで終わらせず、**采配のプロセス**に踏み込む。以下を積極的に拾う:
  - 打順の組み方: 誰を何番に置いたか、その並びに込められた意図（対右/対左、繋ぎ役、一発狙い等）
  - 途中起用: 代打・代走・守備固めを誰にいつ送ったか、その狙い（一球速報に登場する交代選手を根拠に）
  - 継投: 先発を何回で降ろしたか、誰に繋いだか、そのタイミングの判断
  - 相手打順・相手先発との噛み合わせ: カープ打線を相手先発の特徴（左右・球種傾向）にどう当てようとしたか
- aim（狙い）: 打順構成と相手先発への当て方の意図まで具体的に。ただし**断定しない**（「〜を狙ったと見られる」「〜する意図があった可能性」）
  - ★打順変化の深掘り★: 入力の recentLineups（直近3試合の打順）と今日の打順を比較し、**変わった箇所**があれば、なぜその選手をその打順に置いたかの意図を掘る。**変化した箇所だけでなく、狙いを象徴する打順（2番・中軸など）も1〜2箇所、左右(carpBatterHands)や相手先発との相性(keyMatchups)を絡めて言及する**（例:「柳に好相性の秋山を5番に据えた」）。左右・相性の**事実**（誰が左/右か、誰が相手に何割か）は必ず carpBatterHands / keyMatchups で裏取りし、無ければ書かない
  - ★配列の意図まで一歩踏み込む★: 「左右を交互に並べた」「左を上位に固めた」のような**配列パターンを見つけたら、そこで止めず、その配列の戦術的な狙いまで書く**。野球のセオリー（左右交互＝相手が終盤にワンポイント救援＝左殺し/右殺しを出しにくくする／左を固める＝右投手の左への被打率が高い傾向を突く、など）を根拠に、**断定せず「〜狙いと見られる」「〜効果を期待した可能性がある」の形**で。※選手個々の左右は carpBatterHands で裏取りするが、「なぜその配列か」の戦術的理由は一般セオリーに基づく解釈として書いてよい（ただし可能性の表現を保つ）
- pivot（分岐）: 主導権が変わった場面を、代打・継投・守備固めなどの采配プロセスと絡めて。誰の交代・起用がどう試合を動かしたか
- answer（答え）: 狙った采配が機能したかを、実際の打席結果（一球速報/moments）を根拠に。○番に置いた選手が機能したか等
- issue（課題）: 打順・起用・継投・攻め方の面で解決しなかった問題。team_analysis.issues と整合
- next_question: issue から自然につながる「次戦で確かめたい問い」。断定を避け問いの形で
- ★創作禁止の徹底★: 打順・起用・継投・打席結果は入力データ（carpLineup/opponentLineup/pitchers/一球速報）にあるものだけ。**1球ごとの配球（球種・コース）は入力に無いので、具体的な配球（「インコースの直球で」等）は絶対に書かない**。相手先発の球種傾向に触れる場合も、preview/nf3 データにある範囲の一般的傾向に留める
- ★選手の左右★: 打者の左右は **carpBatterHands（例:「右投左打」）に載っている選手のみ** 言及可。載っていない選手の左右は推測禁止。相手先発の左右は opponentLineup や pitchers に明記が無ければ書かない。データがある範囲で「左打者の◯◯を上位に置き…」のように使ってよいが、carpBatterHands で必ず裏取りすること
- ★相手先発との相性★: keyMatchups（今季の対戦成績、例:「秋山 今季柳に5打数4安打.800」）がある場合、それを根拠に打順の意図や結果を語ってよい（例:「柳に好相性の秋山を中軸に据えた」）。keyMatchups に無い選手の相性は創作しない
- 打順の意図は上記の実データ（左右・相性・直近成績）＋打順位置の一般論（1番=出塁/機動力、中軸=長打/得点圏、下位=繋ぎ）で説明する。データの裏付けが無い決めつけはしない
- 全体を通して、事実（何が起きたか）と解釈（意図・意味）を混同しない。首脳陣の意図は複数の可能性がありうる前提で書く
`;
}
...
★【選手名の表記】
order の name は【スタメン組／控え組の調子】に書いてある「短い名前」をそのまま使う。
（例：「大盛」「菊池」「坂倉」「モンテロ」のような短縮形。
  「大盛穂」「菊池涼介」「坂倉将吾」のようなフルネームで書かない。
  nf3 vsカープデータにはフルネームが出てくるが、order には使わない）
...
【出力JSONフォーマット】
{
  ...
  "narrative": {
    "watch_themes": [
      "<今日、試合中に結果を確認できる『問い』の形。40字以内。「〜か」で終える。単なる注目選手紹介ではなく、試合を見て答え合わせできる問い>"
    ],
    "prev_question": "<前回の試合から続く問い、50字以内。前回試合の課題・未解決事項を1つ。前回と関連が薄ければ空文字。勝敗ではなく課題・試み・役割の継続性を優先>"
  }
}

【narrative（観戦テーマ）のルール】
- watch_themes は**1〜3個**。多すぎると情報過多になるので厳選
- 各テーマは「試合中に結果を確認できる問い」にする。例：「若手先発は初回を無失点で入れるか」「絶不調の4番が機能を取り戻せるか」
- 単なる注目選手紹介（「〇〇に注目」）ではなく、必ず**問いの形**にする
- watchpoints / key_players と重複してよい（テーマはそれらを問いに昇華したもの）
- 前日の試合や直近チーム状況と接続する。直近サマリの数値を根拠に
- prev_question: 直近サマリ（特に前回試合）から、まだ解決していない課題を1つ問いにする。前回と関連が薄ければ空文字（""）でよい
- 断定を避け、首脳陣の意図には触れても複数の可能性を残す
